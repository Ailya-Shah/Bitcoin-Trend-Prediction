...
const C = { ink:'#eaecef', accent:'#f7931a', neg:'#f6465d', pos:'#0ecb81', slate:'#58a6ff', line:'#2b3139', soft:'#848e9c', grid:'#20252c' }
...
/* ---------------- live BTC price (Binance WS, REST fallback) ---------------- */
function useLiveBTC() {
  const [t, setT] = useState({ price: null, prev: null, changePct: null, high: null, low: null, live: false })
  const [series, setSeries] = useState([])
  useEffect(() => {
    let ws, poll, alive = true
    const push = (price, changePct, high, low) => {
      if (!Number.isFinite(price)) return
      setT(p => ({ price, prev: p.price, changePct, high: high ?? p.high, low: low ?? p.low, live: true }))
      setSeries(s => [...s.slice(-59), price])
    }
    const startPoll = () => {
      ...
      const go = async () => {
        try {
          ...
          push(+d.lastPrice, +d.priceChangePercent, +d.highPrice, +d.lowPrice)
        } catch {
          try {
            const r = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd&include_24hr_change=true')
            const d = await r.json()
            push(d.bitcoin.usd, d.bitcoin.usd_24h_change, null, null)
          } catch { /* dashboard is fine without the ticker */ }
        }
      }
      go(); poll = setInterval(go, 5000)
    }
    try {
      ws = new WebSocket('wss://stream.binance.com:9443/ws/btcusdt@ticker')
      ws.onmessage = e => { const d = JSON.parse(e.data); push(+d.c, +d.P, +d.h, +d.l) }
      ws.onerror = () => { try { ws.close() } catch {} ; startPoll() }
      ws.onclose = () => { if (alive && !poll) startPoll() }
    } catch { startPoll() }
    return () => { alive = false; try { ws && ws.close() } catch {} ; poll && clearInterval(poll) }
  }, [])
  return { ...t, series }
}

function Sparkline({ data, up }) {
  if (!data || data.length < 2) return <svg className="spark" viewBox="0 0 120 34" />
  const min = Math.min(...data), max = Math.max(...data), rng = (max - min) || 1
  const pts = data.map((v, i) =>
    `${(i / (data.length - 1) * 118 + 1).toFixed(1)},${(31 - (v - min) / rng * 28).toFixed(1)}`).join(' ')
  return (
    <svg className="spark" viewBox="0 0 120 34" preserveAspectRatio="none">
      <polyline points={pts} fill="none" stroke={up ? C.pos : C.neg} strokeWidth="1.5" strokeLinejoin="round" />
    </svg>
  )
}

function LiveTicker() {
  const { price, prev, changePct, high, low, live, series } = useLiveBTC()
  const up = (changePct ?? 0) >= 0
  const flash = price && prev ? (price > prev ? 'up' : price < prev ? 'down' : '') : ''
  return (
    <div className="ticker">
      <div className="tk-pair">
        <span className="tk-dot" data-live={live} />
        <b>BTC</b><span className="tk-quote">/USDT</span>
        <span className="tk-live">{live ? 'LIVE' : '···'}</span>
      </div>
      <div className="tk-price">
        <span key={price} className={`tk-px ${flash}`}>${usd(price)}</span>
        {changePct != null &&
          <span className={`tk-chg ${up ? 'up' : 'down'}`}>{up ? '▲' : '▼'} {Math.abs(changePct).toFixed(2)}%</span>}
      </div>
      <Sparkline data={series} up={up} />
      <div className="tk-hl">
        <div><span>24h High</span><b>${usd(high)}</b></div>
        <div><span>24h Low</span><b>${usd(low)}</b></div>
      </div>
      <div className="tk-tag">live market · <em>research verdict below</em></div>
    </div>
  )
}
...
function Tip({ active, payload, label, suffix='' }) {
  ...
  return (
    <div style={{background:'#1e2329',border:'1px solid #2b3139',color:'#eaecef',padding:'8px 11px',borderRadius:8,fontFamily:'IBM Plex Mono',fontSize:12}}>
      <div style={{color:'#848e9c',marginBottom:3}}>{label}</div>
...
export default function App() {
  ...
  if (err) return <div style={{padding:40,fontFamily:'IBM Plex Mono',color:C.ink}}>Could not load dashboard.json. Run <b>python scripts/export_frontend_data.py</b> first.</div>
        ...
        <div className="chart" style={{height:240}}>
          <ResponsiveContainer>
            <AreaChart data={d.drawdown} margin={{left:6,right:10,top:6}}>
              ...
              <Area dataKey="dd" name="drawdown" stroke={C.neg} fill="#f6465d22" strokeWidth={1.2}/>
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
...
function Models({ d }) {
  ...
  const lo = Math.floor(Math.min(...accs, 50) - 1)
          ...
          <ResponsiveContainer>
            <BarChart data={data} margin={{left:6,right:10,top:10}}>
              ...
              <Bar dataKey="acc" name="accuracy" radius={[4,4,0,0]}>
                {data.map((m,i)=>(<Cell key={i} fill={m.kind==='deep'?'#8b7dd8':C.accent}/>))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
...
function Backtest({ d }) {
  ...
  const colorFor = k => k==='Buy & Hold' ? C.accent : ({ RandomForest:C.pos, XGBoost:C.slate }[k] || '#8b7dd8')
  return (
    <>
...
function Method({ d }) {
  return (
    <>
      ...
      <div className="grid2">
        <div className="panel"><h3>Reproducible</h3><p className="cap" style={{marginBottom:0}}>One command — <code>python run_all.py</code> — executes all nine notebooks in order and regenerates every figure, table, and this dashboard's data.</p></div>
