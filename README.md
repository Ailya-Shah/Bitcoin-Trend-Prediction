...
- **Neural-network benchmark, fairly run.** A neural net is evaluated head-to-head against the classical models on identical chronological splits, and reported honestly even though it loses. By default it trains a scikit-learn **MLP** (no heavy dependencies, so the pipeline always runs); if TensorFlow or PyTorch is installed, the same notebook automatically trains a true **LSTM** instead. The results shown here — and in the live dashboard — are from the MLP.
